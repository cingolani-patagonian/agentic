...
4. **Copy and configure MCP files**
   ...
   - Update `playwright-mcp-config.json`:
     - Find the line containing `"dir": "./media/videos"`
     - Replace it with `"dir": "${WORKTREE_PATH}/media/videos"`
     - Create the media directories: `mkdir -p ${WORKTREE_PATH}/media/{videos,playwright,e2e,review}`
